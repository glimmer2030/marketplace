...
5. **Offer optional PowerPoint generation:**
   ...
   - If the user says **yes**:
     - Use the builtin PPTX generator script:
       - `python3 <plugin_path>/scripts/generate_pptx.py <week_number> <markdown_file> <output_pptx> --template <template_path> --date-range "<start>~<end>"`
       - Example (for this plugin):
         - `python3 /home/william/projects/marketplace/plugins/weekly-report/scripts/generate_pptx.py 2 ./week-2.md ./week-2.pptx --template /home/weijan/projects/weekly-report/template.potx --date-range "2/10~2/21"`
       - If no template.potx is available, omit `--template` to use the fallback (python-pptx from scratch).
...
```bash
python3 <plugin_path>/scripts/generate_pptx.py 2 ./week-2.md ./week-2.pptx --template /home/weijan/projects/weekly-report/template.potx --date-range "1/6~1/17"
```
